...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const [newCode, setNewCode] = useState(`M-ALG-${Date.now().toString().slice(-3)}`);
        ...
        <div className="flex rounded-2xl bg-slate-100 p-1 text-xs font-bold">
          <button
            ...
            className={`px-4 py-2 rounded-xl transition-all flex items-center gap-2 ${
              activeTab === 'payments' ? 'bg-white text-purple-900 shadow-xs' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl transition-all flex items-center gap-2 ${
              activeTab === 'questions' ? 'bg-white text-purple-900 shadow-xs' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
                      ...
                      <td className="py-3.5 px-4 text-right">
                        {payment.status === 'pending' ? (
                          ...
                          <span className="text-[11px] text-slate-400">
                            Processed ({new Date(payment.createdAt).toLocaleDateString()})
                          </span>
                        )}
                      </td>
                    </tr>
