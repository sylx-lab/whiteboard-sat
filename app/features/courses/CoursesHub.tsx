...
function lessonSection(l: Lesson): 'math' | 'reading_writing' {
  return (l.subject as string) === 'reading_writing' ? 'reading_writing' : 'math';
}

function coverUrl(course: Course): string | undefined {
  return course.thumbnailUrl || (course as { posterUrl?: string }).posterUrl || undefined;
}
...
export const CoursesHub: React.FC<CoursesHubProps> = ({
  ...
  const [syllabusTab, setSyllabusTab] = useState<'math' | 'reading_writing'>('math');
  ...
  const handleOpenCourse = (course: Course) => {
    ...
    if (lessons.length > 0) {
      ...
      setSyllabusTab(lessonSection(lessons[0]));
    } else {
    ...
    <div className="bg-(--surface) min-h-[calc(100vh-70px) py-16 px-4 sm:px-6 lg:px-8 space-y-14 animate-in fade-in duration-200">
      ...
      <div className="max-w-310 mx-auto grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
        {courses.map((course, index) => {
          ...
          const poster = coverUrl(course);
          const mathCount = lessonsList.filter((l) => lessonSection(l) === 'math').length;
          const englishCount = lessonsList.filter((l) => lessonSection(l) === 'reading_writing').length;

          return (
            <div
              key={course.id}
              className={`relative rounded-2xl flex flex-col justify-between transition-all duration-200 hover:-translate-y-1 group overflow-hidden ${isFlagship
            ...
            >
              {/* Thumbnail / Poster */}
              {poster ? (
                <div className="relative w-full aspect-[16/9] overflow-hidden bg-(--surface-soft) shrink-0">
                  <img src={poster} alt={`${course.title} poster`} className="w-full h-full object-cover" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/15 to-transparent pointer-events-none" />
                  {/* Corner index over image */}
                  <div className="absolute -top-1 right-2 pointer-events-none select-none text-[64px] font-extrabold font-mono text-white/25 leading-none">
                    {cardIndexStr}
                  </div>
                  <div className={`absolute bottom-2 right-2 px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider backdrop-blur-md ${isFlagship ? 'bg-(--brand-cta) text-white' : 'bg-white/90 text-(--foreground) border border-white/50'}`}>
                    {course.badge || course.level || 'COMPREHENSIVE'}
                  </div>
                </div>
              ) : (
              ...
              {isFlagship && (
                <div className="absolute top-0 left-0 right-0 h-0.5 bg-(--brand) rounded-t-2xl z-10" />
              )}
              ...
              <div className="p-7 space-y-5 flex-1 flex flex-col justify-between relative z-10">
                <div className="space-y-5">
                  ...
                  <div className="flex items-center justify-between">
                    <span
                      className={`px-3 py-1 rounded-md text-[11.5px] font-semibold tracking-wide uppercase ${poster
                        ? 'bg-(--surface-soft) text-(--foreground) border border-(--border)'
                        : isFlagship
                    ...
                    >
                      {poster ? `${mathCount} Math • ${englishCount} English` : isFlagship ? 'BEST VALUE' : course.badge || course.level || 'COMPREHENSIVE'}
                    </span>
      ...
      {selectedCourse && (() => {
        ...
        const mathLessons = allLessons.filter((l) => lessonSection(l) === 'math');
        const englishLessons = allLessons.filter((l) => lessonSection(l) === 'reading_writing');
          ...
          <div className="bg-(--surface) rounded-2xl shadow-xl border border-(--border) w-full max-w-5xl max-h-[92vh] flex flex-col overflow-hidden">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-(--foreground-secondary)">
                    {selectedCourse.instructorName || 'White Board Faculty'} • {selectedCourse.lessonsCount || allLessons.length || 0} Classes
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="flex-1 overflow-y-auto grid grid-cols-1 lg:grid-cols-12">
                      ...
                      {activeLesson.videoUrl ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center">
                          <div className="text-center space-y-2 p-6 text-white">
                            <div className="w-12 h-12 rounded-full bg-(--brand-cta) text-white flex items-center justify-center mx-auto shadow-md">
                              <Play className="w-5 h-5 fill-white ml-0.5" />
                            </div>
                            <div className="font-bold text-[13px]">{activeLesson.title}</div>
                            <div className="text-[11px] text-white/60 font-mono">Duration: {activeLesson.durationMinutes} Minutes • HD 1080p</div>
                          </div>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-1 gap-3">
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border ${lessonSection(activeLesson) === 'math' ? 'bg-teal-50 text-teal-800 border-teal-200' : 'bg-indigo-50 text-indigo-800 border-indigo-200'}`}>
                            {lessonSection(activeLesson) === 'math' ? <Calculator className="w-3 h-3" /> : <Languages className="w-3 h-3" />}
                            {lessonSection(activeLesson) === 'math' ? 'Math' : 'English'}
                          </span>
                          {activeLesson.isFreePreview && <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-50 text-emerald-800 border border-emerald-200 uppercase tracking-wider">Free Preview</span>}
                        </div>
                        <h4 className="font-bold text-(--foreground) text-base mt-1">{activeLesson.title}</h4>
                      ...
                      {hasAccessToCourse(selectedCourse.id) && (
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-lg text-[11px] font-medium border transition-colors flex items-center gap-1.5 cursor-pointer shrink-0 ${(courseProgress[selectedCourse.id] || []).includes(activeLesson.id)
              ...
              <div className="lg:col-span-4 bg-(--brand-soft) p-5 overflow-y-auto space-y-4">
                ...
                {/* Math / English tabs */}
                <div className="flex items-center gap-1 p-1 bg-(--surface) rounded-xl border border-(--border) w-full">
                  <button
                    onClick={() => setSyllabusTab('math')}
                    className={`flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] font-semibold transition-colors cursor-pointer ${syllabusTab === 'math' ? 'bg-(--brand-cta) text-white shadow-2xs' : 'text-(--foreground-secondary) hover:bg-(--surface-soft)'}`}
                  >
                    <Calculator className="w-3.5 h-3.5" />
                    Math
                    <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${syllabusTab === 'math' ? 'bg-white/20 text-white' : 'bg-(--brand-soft) text-(--brand-text)'}`}>{mathLessons.length}</span>
                  </button>
                  <button
                    onClick={() => setSyllabusTab('reading_writing')}
                    className={`flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] font-semibold transition-colors cursor-pointer ${syllabusTab === 'reading_writing' ? 'bg-(--brand-cta) text-white shadow-2xs' : 'text-(--foreground-secondary) hover:bg-(--surface-soft)'}`}
                  >
                    <Languages className="w-3.5 h-3.5" />
                    English
                    <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${syllabusTab === 'reading_writing' ? 'bg-white/20 text-white' : 'bg-(--brand-soft) text-(--brand-text)'}`}>{englishLessons.length}</span>
                  </button>
                </div>

                <div className="text-[11px] text-(--foreground-secondary) font-mono">
                  {activeLessons.length} {syllabusTab === 'math' ? 'Math' : 'English'} classes • {activeLessons.reduce((s, l) => s + (Number(l.durationMinutes) || 0), 0)} mins
                </div>

                <div className="space-y-2">
                  {activeLessons.length === 0 ? (
                    <div className="py-8 text-center rounded-xl border border-dashed border-(--border) bg-(--surface) space-y-2 px-3">
                      <div className="w-8 h-8 rounded-lg bg-(--brand-soft) text-(--brand-text) grid place-items-center mx-auto">
                        {syllabusTab === 'math' ? <Calculator className="w-4 h-4" /> : <Languages className="w-4 h-4" />}
                      </div>
                      <p className="text-[12px] font-medium text-(--foreground)">No {syllabusTab === 'math' ? 'Math' : 'English'} classes yet</p>
                      <p className="text-[11px] text-(--foreground-secondary)">This course has no classes in the {syllabusTab === 'math' ? 'Math' : 'English'} section.</p>
                    </div>
                  ) : (
